...
create<-function(trait,exclude_all_both=NULL,exclude_all_cases=NULL,exclude_all_controls=NULL,
                 ...
                 pheno_path,extension='.tab.tsv'){
  ##phenotype file
  a<-fread(paste0(pheno_path,trait,extension),header=T) #503629
  ...
  ##exclusion files
  drop<-fread("/medpop/afib/skhurshid/phenotypes/withdrawals/w7089_20210201.csv",header=F)
  ...
  for (i in exclude_all_both){
    exclude <- fread(paste0(pheno_path,i,extension),header=T)
