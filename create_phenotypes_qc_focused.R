...
create<-function(trait,exclude_all_both=NULL,exclude_all_cases=NULL,exclude_all_controls=NULL,
  ...
  ##phenotype file
  a<-fread(paste0("/medpop/esp2/projects/UK_Biobank/Phenotype_Library/phenoV2/disease/",trait,'.tab.tsv'),header=T) #503629
  setnames(a,'has_disease',trait)
  a<-a[!is.na(trait)]
  print(paste0('Total N:',nrow(a)))
  
  ##sample qc file
  b<-fread("/medpop/esp2/pradeep/UKBiobank/v2data/ukb_sqc_v2_7089.tsv",header=T) ##488377
  ...
  white<-fread("/medpop/afib/chaffin/ukbb_pca/UKB.BritishIrishOtherWhite.inclusion.list.v1.txt",header=T)
  ab[,':='(white = ifelse(sample_id %in% white$eid,1,0))]
  
  ##exclusion files
  drop<-fread("/medpop/afib/data/ukb9389/exclusion/w17488_20200204.csv",header=F)
  link<-fread("/medpop/afib/data/ukb9389/ukb_app17488_app7089_link.csv",header=T)
  ...
  rel<-fread("/medpop/esp2/pradeep/UKBiobank/v2data/ukb708_rel_chr1_s488374.dat",header=T)
